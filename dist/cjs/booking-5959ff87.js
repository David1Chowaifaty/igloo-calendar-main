...
async function getStayStatus() {
  try {
    const token = axios.calendar_data.token;
    if (token) {
...
function renderBlock003Date(date, hour, minute) {
  ...
  return `${axios.locales.entries.Lcz_BlockedTill} ${moment.hooks(dt).format('MMM DD, HH:mm')}`;
}
...
function updateBookingWithStayData(data, cell) {
  data.NO_OF_DAYS = utils.dateDifference(data.FROM_DATE, cell.DATE);
...
//# sourceMappingURL=booking-5959ff87.js.map