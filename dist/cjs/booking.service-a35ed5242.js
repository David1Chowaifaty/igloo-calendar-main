...
class BookingService extends axios.Token {
  async getCalendarData(propertyid, from_date, to_date) {
    try {
      const token = this.getToken();
      if (token !== null) {
        const { data } = await axios.axios.post(`/Get_Exposed_Calendar?Ticket=${token}`, {
          propertyid,
          from_date,
          to_date,
        });
  ...
  async fetchGuest(email) {
    try {
      const token = this.getToken();
      if (token !== null) {
        const { data } = await axios.axios.post(`/Get_Exposed_Guest?Ticket=${token}`, { email });
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
        }
  ...
  async editExposedGuest(guest, book_nbr) {
    try {
      const token = this.getToken();
      if (token !== null) {
        const { data } = await axios.axios.post(`/Edit_Exposed_Guest?Ticket=${token}`, Object.assign(Object.assign({}, guest), { book_nbr }));
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
        }
        return data.My_Result;
  ...
  async getBookingAvailability(from_date, to_date, propertyid, adultChildCount, language, room_type_ids, currency) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_Exposed_Booking_Availability?Ticket=${token}`, {
          propertyid,
          from_date,
          to_date,
  ...
  async getBlockedInfo() {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_Setup_Entries_By_TBL_NAME_MULTI?Ticket=${token}`, { TBL_NAMES: ['_CALENDAR_BLOCKED_TILL'] });
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
        }
        return data.My_Result;
  ...
  async blockUnit(params) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Block_Exposed_Unit?Ticket=${token}`, params);
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
        }
        console.log(data);
  ...
  async getUserInfo(email) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/GET_EXPOSED_GUEST?Ticket=${token}`, {
          email,
        });
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
  ...
  async getExposedBooking(booking_nbr, language) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_Exposed_Booking?Ticket=${token}`, {
          booking_nbr,
          language,
        });
        if (data.ExceptionMsg !== '') {
  ...
  async fetchExposedGuest(email, property_id) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Fetch_Exposed_Guests?Ticket=${token}`, {
          email,
          property_id,
        });
        if (data.ExceptionMsg !== '') {
  ...
  async fetchExposedBookings(booking_nbr, property_id, from_date, to_date) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Fetch_Exposed_Bookings?Ticket=${token}`, {
          booking_nbr,
          property_id,
          from_date,
          to_date,
  ...
  async getPCICardInfoURL(BOOK_NBR) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_PCI_Card_Info_URL?Ticket=${token}`, {
          BOOK_NBR,
        });
        if (data.ExceptionMsg !== '') {
          throw new Error(data.ExceptionMsg);
...
//# sourceMappingURL=booking.service-a35ed5242.js.map