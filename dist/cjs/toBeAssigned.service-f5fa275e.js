...
class ToBeAssignedService extends axios.Token {
  async getUnassignedDates(propertyid, from_date, to_date) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_UnAssigned_Dates?Ticket=${token}`, {
          propertyid,
          from_date,
          to_date,
        });
  ...
  async getUnassignedRooms(propertyid, specific_date, roomInfo, formattedLegendData) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Get_Aggregated_UnAssigned_Rooms?Ticket=${token}`, {
          propertyid,
          specific_date,
        });
        if (data.ExceptionMsg !== '') {
  ...
  async assignUnit(booking_nbr, identifier, pr_id) {
    try {
      const token = this.getToken();
      if (token) {
        const { data } = await axios.axios.post(`/Assign_Exposed_Room?Ticket=${token}`, {
          booking_nbr,
          identifier,
          pr_id,
        });
...
//# sourceMappingURL=toBeAssigned.service-f5fa275e.js.map