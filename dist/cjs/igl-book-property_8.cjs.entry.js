...
const irInterceptor_store = require('./ir-interceptor.store-f87bf87e.js');
...
const IrInterceptor = class {
  ...
  render() {
    return (index.h(index.Host, null, this.isLoading && (index.h("div", { class: "loadingScreenContainer" }, index.h("div", { class: "loadingContainer" }, index.h("ir-loading-screen", null))))));
  }
};
...
const IrPaymentDetails = class {
  ...
  async componentWillLoad() {
    try {
      this.paymentService.setToken(axios.calendar_data.token);
...
class PickupService {
  async savePickup(params, booking_nbr, is_remove) {
    try {
      ...
      await axios.axios.post(`/Do_Pickup?Ticket=${axios.calendar_data.token}`, {
        booking_nbr,
        is_remove,
  ...
  getAvailableLocations(message) {
    let locations = [];
    axios.calendar_data.pickup_service.allowed_options.forEach(option => {
  ...
  getPickUpPersonStatus(code) {
    const getCodeDescription = axios.calendar_data.pickup_service.allowed_pricing_models.find(model => model.code === code);
...
const IrPickup = class {
  ...
  componentWillLoad() {
    if (this.defaultPickupData) {
      ...
      this.allowedOptionsByLocation = axios.calendar_data.pickup_service.allowed_options.filter(option => option.location.id === transformedData.location);
  ...
  handleLocationChange(event) {
    ...
    if (value !== '') {
      this.allowedOptionsByLocation = axios.calendar_data.pickup_service.allowed_options.filter(option => option.location.id.toString() === value);
  ...
  handleVehicleTypeChange(e) {
    ...
    let locationChoice = axios.calendar_data.pickup_service.allowed_options.find(option => option.location.id === +this.pickupData.location && option.vehicle.code === e.detail);
    if (!locationChoice) {
      return;
    }
    this.vehicleCapacity = [...this.pickupService.getNumberOfVehicles(locationChoice.vehicle.capacity, this.numberOfPersons)];
...
const irRoomCss = ".light-blue-bg.sc-ir-room{background:#acecff;padding:0.2rem 0.3rem;border-radius:5px}.payment-container.sc-ir-room{position:absolute;right:1rem;height:fit-content}.sc-ir-room-h{position:relative}.sm-mr.sc-ir-room{margin-right:3px}.mx-0-5.sc-ir-room{margin-left:2px !important;margin-right:2px !important}.tax-width.sc-ir-room{font-size:10px}.sm-mb-1.sc-ir-room{margin-bottom:5px !important}.sm-mt-1.sc-ir-room{margin-top:5px !important}";
